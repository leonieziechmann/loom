...
#test-case(
  ...
  #let child = motif(
    measure: (ctx, _) => {
      let check1 = assert-eq(ctx.at("test-flag", default: none), 12345, msg: "Child sees parent scope")
...
#test-case(
  ...
  #let checker(key, should-exist) = motif(
    measure: (ctx, _) => {
      ...
      let check1 = assert-eq(has-key, should-exist, msg: "Key `" + key + "` visibility correct")
      return (none, (check1,))
    },
...
#test-case(
  ...
  #let check-path(expected-current, expected-parent) = motif(
    measure: (ctx, _) => {
      let check1 = assert-eq(path.current-kind(ctx), expected-current, msg: "Current path is " + expected-current)
      let check2 = assert-eq(path.parent-kind(ctx), expected-parent, msg: "Parent path is " + expected-parent)
...
#test-case(
  ...
  #content-motif(draw: (ctx, body) => [
    ...
    #assert-eq(data-motif.signal.at("val", default: 0), 1, msg: "Data Signal was propagated correctly.")
...
#test-case(
  ...
  #let check-theme(expected) = motif(
    measure: (ctx, _) => {
      ...
      let check = assert-eq(actual, expected, msg: "Context theme is " + expected)
      return (none, (check,))
    },
    draw: (.., view, _) => view.join(parbreak()),
    none
